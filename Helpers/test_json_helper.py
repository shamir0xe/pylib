...
class TestJsonHelper(unittest.TestCase):
    ...
    def test_selector_set_value_index_array(self):
        ...
        next_js = JsonHelper.selector_set_value(
            self.js, 
            "some_array.__{}.b".format(random_index), 
            random_value)
        bl = next_js["some_array"][random_index]["b"] == random_value
        bl &= not "b" in next_js["some_array"][1 - random_index] or \
            next_js["some_array"][1 - random_index]["b"] != random_value
        self.assertTrue(bl)
    ...
    def test_apply_structure_list(self):
        next_js = JsonHelper.apply_stucture(self.js, {
            "some_array.__1": []
        })
