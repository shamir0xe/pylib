...
class TerminalProcess:
    def __init__(self, total_loops):
        ...
        self.trigger = 0.05
        self.last = -1e10
        self.__bar_length = 20
        print("", flush=True)

    def __bar(self, progress):
        filled_ones = math.ceil(progress * self.__bar_length)
        res = "#" * filled_ones + "_" * (self.__bar_length - filled_ones)
        return res

    def flush(self):
        progress = self.turn / self.total
        print("\r[{}]\r\t\t\t{:02d}%".format(
            self.__bar(progress), min(100, round(1. * progress * 100))),
              end="\r\033[A\n",
              flush=True)

    def hit(self):
        self.turn += 1
        if self.turn - self.last >= self.trigger * self.total:
            self.last = self.turn
            self.flush()

    def completed(self):
        return self.turn >= self.total
